...
#raw data processing
if(T){
  out_dir="/path/to/PT8_RDS"
  system(paste0("mkdir -p ",out_dir))
  tc.data <- Read10X(data.dir = "/path/to/PT8_GEX/outs/filtered_feature_bc_matrix/")
...
assign_cell_type <- function(numb){
  if (numb %in% c(9)){
    clust = 'myofibroblast'
  } else if (numb %in% c(18)){
    clust = 'fibroblast'
  }else if (numb %in% c(11,20)){
    clust = 'endothelial_cell'
  } else if (numb %in% c(0,1,4,12,21,22)){
    clust= 'ptc'
  } else if (numb %in% c(2,3,13,16,17)){
    clust = 'macrophage'
    ...
    clust = 'Treg_cell'
  } else if (numb %in% c(14,6)){
    clust = 'B_cell'
  } else if (numb %in% c(19)){
    clust = 'plasma_cell'
  } else if (numb %in% c()){
    clust = 'mast_cell'
  } else if (numb %in% c(5,10)){
    clust = 'low_qual'
  } else if (numb %in% c()){
    clust = 'doublet' 
  } else{
