...
immune_genes=c('PTPRC','CD68','CD14','CD4','CD8A','NKG7','CD79A','CD79B','IGHG1','TPSAB1')


#raw data processing
if(T){
  out_dir="/path/to/PT5_RDS"
  tc.data <- Read10X(data.dir = "/path/to/PT5_GEX/outs/filtered_feature_bc_matrix/")
  ...
  if(T){
    ...
    plot1 <- FeatureScatter(tc, feature1 = "nCount_RNA", feature2 = "percent.mt")
  ...
  #Idents(tc) <- 'RNA_snn_res.0.5'
  Idents(tc) <- 'RNA_snn_res.1'
  #Idents(tc) <- 'RNA_snn_res.1.5'
  
  if(T){
    DimPlot(tc, reduction = "umap")
    FeaturePlot(tc, features='percent.mt')
    FeaturePlot(tc, features='nFeature_RNA')
  ...
  if(T){
    sweep.res.list_tc <- paramSweep_v3(tc, PCs = 1:30, sct = FALSE)
  }
  ...
  tc <- doubletFinder_v3(tc, PCs = 1:30, pN = 0.25, pK = 25, nExp = nExp_poi.adj, reuse.pANN = FALSE, sct = FALSE)
  tc@meta.data %>% colnames()
  #Idents(tc) <- 'DF.classifications_0.25_0.09_760'
  Idents(tc) <- 'DF.classifications_0.25_25_760'
  DimPlot(tc, reduction = "umap")
  
  
  #custom cluster identification
  if(T){
    Idents(tc) <- 'RNA_snn_res.1'
    DefaultAssay(tc) <- 'RNA'
    #0 vs 1,2,3
    mk_0vs1_2_3 <- FindMarkers(tc, ident.1 = c(0), ident.2=c(1,2,3), min.pct = 0.25, max.cells.per.ident=2000, random.seed=1)
    tmp_feature = rownames(mk_0vs1_2_3)[1:9]
    
    FeaturePlot(tc, features=tmp_feature, slot='data')
    FeaturePlot(tc, features=tmp_feature2, slot='data')
    
    mk_8vs16 <- FindMarkers(tc, ident.1 = c(8), ident.2=c(16), min.pct = 0.25, max.cells.per.ident=2000, random.seed=1)
    tmp_feature = rownames(mk_8vs16)[1:9]
    tmp_feature2 = rownames(mk_8vs16)[10:18]
    FeaturePlot(tc, features=tmp_feature, slot='data')
    FeaturePlot(tc, features=tmp_feature2, slot='data')
  }
 
  # cell filter after clustering
  Idents(tc) <- 'RNA_snn_res.1'
  tc <- subset(tc, idents = setdiff(unique(Idents(tc)), c(0,6,10,16)))
  DimPlot(tc, reduction = "umap")
...
assign_cell_type <- function(numb){
  if (numb %in% c(20)){
    clust = 'myofibroblast'
  } else if (numb %in% c(19)){
    clust = 'fibroblast'
  } else if (numb %in% c(4,11,7,17,22)){
    clust='macrophage'
  } else if (numb %in% c(13)){
    clust = 'endothelial_cell'
  } else if (numb %in% c(8)){
    clust= 'ptc'
  } else if (numb %in% c(5,18,23)){
    clust = 'plasma_cell'
  } else if (numb %in% c(15)){
    clust = 'B_cell'
  } else if (numb %in% c(3,9)){
    clust = 'CD4_T_cell'
  } else if (numb %in% c(1,2,12)){
    clust = 'CD8_T_NK_cell'
  } else if (numb %in% c(14)){
    clust = 'proliferating_immune'
  }else if (numb %in% c(21)){
    clust = 'unknown_macrophage'
  }else if (numb %in% c(0,6,10,16)){
    clust = 'low_qual'
  }else{
    clust='unknown'
  }
...
tmp_meta <- tmp_meta %>% rowwise() %>% mutate(cell_type = assign_cell_type(RNA_snn_res.1))
