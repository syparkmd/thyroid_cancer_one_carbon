...
#raw data processing
if(T){
  out_dir="/path/to/PT12_RDS"
  system(paste0("mkdir -p ",out_dir))
  tc.data <- Read10X(data.dir = "/path/to/PT12_GEX/outs/filtered_feature_bc_matrix/")
...
assign_cell_type <- function(numb){
  if (numb %in% c()){
    ...
    clust = 'macrophage'
  } else if (numb %in% c(13,20)){
    clust = 'NK_cell'
  } else if (numb %in% c(1,8,9)){
    clust = 'CD8_T_cell'
  } else if (numb %in% c(5,12)){
    clust = 'CD4_T_cell'
  } else if (numb %in% c(3)){
    clust = 'Treg_cell'
  } else if (numb %in% c(0)){
    clust = 'naive_T_cell'
  } else if (numb %in% c(2,4,18,21)){
    clust = 'B_cell'
  } else if (numb %in% c(6,10,17,22)){
    clust = 'plasma_cell'
  } else if (numb %in% c()){
    clust = 'mast_cell'
  } else if (numb %in% c(14,19)){
    clust = 'low_qual'
  } else if (numb %in% c()){
    clust = 'doublet' 
  } else{
