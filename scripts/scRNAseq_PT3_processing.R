...
#raw data processing
if(T){
  out_dir="/path/to/PT3_RDS"
  system(paste0("mkdir -p ",out_dir))
  tc.data <- Read10X(data.dir = "/path/to/PT3_GEX/outs/filtered_feature_bc_matrix/")
  ...
  if(T){
    ...
    Idents(tc) <- 'RNA_snn_res.1'  #final resolution selection
...
assign_cell_type <- function(numb){
  if (numb %in% c(5)){
    clust = 'myofibroblast'
  } else if (numb %in% c(11)){
    clust = 'fibroblast'
  }else if (numb %in% c(9,10,13)){
    clust = 'endothelial_cell'
  } else if (numb %in% c(1,2,3,0,8,20,25)){
    clust= 'ptc'
  }else if (numb %in% c()){
    clust = 'normal_thyroid'
  } else if (numb %in% c(4,12,15,16,18,23)){
    clust = 'macrophage'
