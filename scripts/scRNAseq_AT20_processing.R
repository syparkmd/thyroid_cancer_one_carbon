...
#raw data processing
if(T){
  out_dir="/path/to/AT20_RDS"
  system(paste0("mkdir -p ",out_dir))
  tc.data <- Read10X(data.dir = "/path/to/Thy20_GEX/outs/filtered_feature_bc_matrix/")
...
assign_cell_type <- function(numb){
  if (numb %in% c(21)){
    clust = 'myofibroblast'
  } else if (numb %in% c(12,16,17,20)){
    clust = 'fibroblast'
  }else if (numb %in% c(5)){
    clust = 'endothelial_cell'
  } else if (numb %in% c(1,8,15,18,19)){
    clust= 'atc'
  }else if (numb %in% c()){
    clust = 'normal_thyroid'
  } else if (numb %in% c(2,3,10,11,13,29)){
    clust = 'macrophage'
  } else if (numb %in% c(9,22)){
    clust = 'NK_cell'
  }else if (numb %in% c(6)){
