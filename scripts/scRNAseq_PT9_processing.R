...
#raw data processing
if(T){
  out_dir="/path/to/PT9_RDS"
  system(paste0("mkdir -p ",out_dir))
  tc.data <- Read10X(data.dir = "/path/to/PT9_GEX/outs/filtered_feature_bc_matrix/")
...
assign_cell_type <- function(numb){
  if (numb %in% c(6)){
    ...
    clust = 'fibroblast'
  }else if (numb %in% c(9,12,24)){
    clust = 'endothelial_cell'
  } else if (numb %in% c(2,19)){
    clust= 'ptc'
  } else if (numb %in% c(1,23,26,27,21,11,8)){
    clust = 'macrophage'
  } else if (numb %in% c(17,20)){
    clust = 'NK_cell'
  } else if (numb %in% c(5,15)){
    clust = 'CD8_T_cell'
  } else if (numb %in% c(3)){
    clust = 'CD4_T_cell'
    ...
    clust = 'plasma_cell'
  } else if (numb %in% c(25)){
    clust = 'mast_cell'
  } else if (numb %in% c(0,7,10,18,4,22)){
    clust = 'low_qual'
  } else if (numb %in% c()){
    clust = 'doublet' 
  } else{
