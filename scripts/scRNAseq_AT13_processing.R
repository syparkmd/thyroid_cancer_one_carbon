...
#raw data processing
if(T){
  out_dir="/path/to/AT13_RDS"
  system(paste0("mkdir -p ",out_dir))
  tc.data <- Read10X(data.dir = "/path/to/Thy13_GEX/outs/filtered_feature_bc_matrix/")
...
"
Fibro_myofibro:18,22
Endo:16
Atc:25
Macro:4,8,10,12,17,24
Treg:5
CD4:0,13,32
CD8:3,7,19
NK:20,2,31
...
assign_cell_type <- function(numb){
  if (numb %in% c()){
    ...
    clust = 'fibroblast'
  } else if (numb %in% c(18,22)){
    clust = 'fibroblast_myofibroblast'
  ...
  } else if (numb %in% c(4,8,10,12,17,24,26,28)){
    clust = 'macrophage'
  } else if (numb %in% c(20,2,31)){
    clust = 'NK_cell'
  }else if (numb %in% c()){
    clust = 'unknown_T_cell'
  } else if (numb %in% c(3,7,19)){
    clust = 'CD8_T_cell'
  } else if (numb %in% c(0,13,32)){
    clust = 'CD4_T_cell'
  } else if (numb %in% c(5)){
    clust = 'Treg_cell'
  } else if (numb %in% c()){
    ...
    clust = 'plasma_cell'
  } else if (numb %in% c(25,29)){
    clust = 'mast_cell'
  } else if (numb %in% c( 1,11,14,15,6,21,27)){
    clust = 'low_qual'
