...
#raw data processing
if(T){
  out_dir="/path/to/AT9_RDS"
  system(paste0("mkdir -p ",out_dir))
  tc.data <- Read10X(data.dir = "/path/to/Thy09_GEX/outs/filtered_feature_bc_matrix/")
...
assign_cell_type <- function(numb){
  if (numb %in% c(0,8,12)){
    ...
    clust = 'fibroblast'
  }else if (numb %in% c(5,7,9)){
    clust = 'endothelial_cell'
  } else if (numb %in% c(1,3,4,6,13,17)){
    clust= 'atc'
  }else if (numb %in% c()){
    clust = 'normal_thyroid'
  } else if (numb %in% c(2,10)){
    clust = 'macrophage'
