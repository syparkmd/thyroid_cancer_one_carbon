...
#raw data processing
if(T){
  out_dir="/path/to/PT7_RDS"
  system(paste0("mkdir -p ",out_dir))
  tc.data <- Read10X(data.dir = "/path/to/PT7_GEX/outs/filtered_feature_bc_matrix/")
  ...
  if(F){
    ...
    DimPlot(tc)
    draw_umap_plot(tc,'RNA_snn_res.1' )
   
    FeaturePlot(tc, features='percent.mt')
    FeaturePlot(tc, features='nFeature_RNA')
    FeaturePlot(tc, 'MKI67', slot='data')
    
    tmp_meta <- tc@meta.data %>% as.data.frame() %>% rownames_to_column('cell_id') %>% as_tibble()
    tmp1 <- tmp_meta %>% group_by(`RNA_snn_res.1`) %>% summarise(mean_feature = mean(nFeature_RNA))
    tmp2 <- tmp_meta %>% group_by(`RNA_snn_res.1`) %>% summarise(mean_mito = mean(percent.mt))
    
    ggplot(tmp2, aes(x='',y=mean_mito))+
      geom_boxplot()+
      geom_point(color='red')+
      geom_text_repel(aes(label=`RNA_snn_res.1`), size=7, max.overlaps=30)+
      xlab('')
    
    ggplot(tmp1, aes(x='',y=mean_feature))+
      geom_boxplot()+
      geom_point(color='red')+
      geom_text_repel(aes(label=`RNA_snn_res.1`), size=7, max.overlaps=30)+
      xlab('')
...
ggplot(tmp_dt, aes(x=avg_logFC,y=-log10(p_val_adj)))+
  geom_point()+
  geom_text_repel(aes(label=gene_name))+
  xlab(paste0(paste(g2,collapse=','),'<----->',g1))
#GSEA
...
ggplot(f_res, aes(x=short_name, y=NES, fill= pval))+
  geom_bar(stat='identity')+
  geom_text(aes(y=5, label=rep_genes), hjust=0)+
  scale_y_continuous(limits=c(-5,15))+
  scale_x_discrete(limits = x_order)+
  coord_flip()+
  ylab(paste0(paste(g2,collapse=','),'<----->',g1))
...
assign_cell_type <- function(numb){
  if (numb %in% c(11,15)){
    clust = 'myofibroblast'
  } else if (numb %in% c(20)){
    clust = 'fibroblast'
  }else if (numb %in% c(14,3,18,28)){
    clust = 'endothelial_cell'
  } else if (numb %in% c(0,2,6)){
    clust= 'ptc'
  } else if (numb %in% c(23)){
    clust= 'normal_thyroid'
  }else if (numb %in% c(9,21,25,4,22,12,27)){
    clust = 'macrophage'
    ...
    clust = 'CD4_T_cell'
  } else if (numb %in% c(17)){
    clust = 'Treg_cell'
  } else if (numb %in% c(16)){
    clust = 'B_cell'
  } else if (numb %in% c(26,29)){
    clust = 'plasma_cell'
  } else if (numb %in% c(30)){
    clust = 'mast_cell'
  } else if (numb %in% c(1, 5,8,19)){
    clust = 'low_qual'
  } else if (numb %in% c(24)){
    clust = 'doublet' 
  } else{
    clust='unknown'
  }
