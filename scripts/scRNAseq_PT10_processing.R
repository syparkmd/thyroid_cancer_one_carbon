...
#raw data processing
if(T){
  out_dir="/path/to/PT10_RDS"
  system(paste0("mkdir -p ",out_dir))
  tc.data <- Read10X(data.dir = "/path/to/PT10_GEX/outs/filtered_feature_bc_matrix/")
...
assign_cell_type <- function(numb){
  if (numb %in% c(15)){
    ...
    clust = 'fibroblast'
  }else if (numb %in% c(3,16)){
    clust = 'endothelial_cell'
  } else if (numb %in% c(8,18)){
    clust= 'ptc'
  }else if (numb %in% c(14)){
    clust = 'normal_thyroid'
  } else if (numb %in% c(11,4,9,21,22,23)){
    clust = 'macrophage'
  } else if (numb %in% c(10)){
    clust = 'NK_cell'
  } else if (numb %in% c(2,13)){
    clust = 'CD8_T_cell'
  } else if (numb %in% c(0,5)){
    clust = 'CD4_T_cell'
  } else if (numb %in% c(1,7)){
    clust = 'Treg_cell'
  } else if (numb %in% c(12)){
    clust = 'B_cell'
  } else if (numb %in% c()){
    clust = 'plasma_cell'
  } else if (numb %in% c(20)){
    clust = 'mast_cell'
  } else if (numb %in% c(6,19)){
    clust = 'low_qual'
  } else if (numb %in% c()){
    clust = 'doublet' 
  } else{
